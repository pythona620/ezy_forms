    ...
    <div class="offcanvas addOffCanvas offcanvas-end" tabindex="-1" id="offcanvasRight"
      ...
      <div class="offcanvas-header add_designationHeader">
        <span id="offcanvasRightLabel" class="font-14 fw-bold">
          {{ selectedBlockIndex == 0 ? "Add Requestors" : `Approval Settings - Level ${selectedBlockIndex}` }}
        </span>
      ...
      <div class="offcanvas-body add_desig_offCanvas_body p-0">
        <div class="">
          <!-- Approver Type (for approvers only) -->
...
const handleWFRoadMapSelect = async (selectedRoadMap) => {
  try {
    ...
    if (roadmapData.wf_requestors && roadmapData.wf_requestors.length > 0) {
      ...
      roadmapData.wf_requestors.forEach((req, index) => {
        workflowSetup.push({
          type: 'requestor',
          roles: [req.requestor],
    ...
    if (roadmapData.wf_level_setup && roadmapData.wf_level_setup.length > 0) {
      ...
      sortedLevels.forEach((lvl) => {
        ...
        // Create approver block
        addBlock();

        // Add to workflowSetup (one entry per level, multiple roles if needed)
    ...
    showSuccess(`Workflow loaded: ${loadedRequestors.value.length} requestor(s) and ${loadedApprovers.value.length} approver level(s). Blocks created in Step 2.`);
  } catch (error) {
