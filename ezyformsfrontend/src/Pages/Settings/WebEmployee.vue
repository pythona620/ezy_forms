...
const exportEmployeesToExcel = async () => {
  const filters = [["company_field", "=", `${newbusiness.value}`], ["is_web_form", "=", "1"]];
  const queryParams = {
...
function employeeData(data) {
  const filters = [["company_field", "=", `${newbusiness.value}`], ["is_web_form", "=", "1"], ["enable", "=", "0"]];
  if (data) {
    filters.push(...data);
  }
    ...
    .then((res) => {
      if (res.message.data) {
        ...
        if (filterObj.value.limit_start === 0) {
          ...
          // designations.value = [...new Set(res.message.data.map((designation) => designation.designation))];
          reportingTo.value = [
