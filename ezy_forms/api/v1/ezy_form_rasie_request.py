...
def process_individual_requests(ids, doctype_name, doctype_field, property, cluster, 
	...
	sending_mail_api(request_id=request_id, doctype_name=doctype_name,property= property,cluster= cluster,reason= reason,timestamp= timestamp,skip_user_role= None,user=None,field_changes=None,current_level=None )
	...
	try:
		...
		for level in range(1,int(road_map)+1):
			...
			if approvar_excits and not status:
				...
				if has_match:
					...
					if not requester_as_a_approver:
						...
						frappe.db.commit()
						sending_mail_api(request_id=request_id, doctype_name=document_type,property= property,cluster= cluster_name,reason= "Auto Approved by the system",timestamp= my_time,skip_user_role= match_role,user = match_role_user,field_changes=None,current_level=None )
				picking_remaining_roles_for_approval = [remaining_role["role"]  for remaining_role in approvals_reasons  if int(remaining_role["level"]) == int(current_level) and not remaining_role["action"].strip() and not remaining_role["user"].strip() ]
			else:
				picking_remaining_roles_for_approval = [remaining_role["role"]  for remaining_role in approvals_reasons  if int(remaining_role["level"]) == int(current_level) and not remaining_role["action"].strip() and not remaining_role["user"].strip() ]
			view_only_roles = [
			...
			if current_user_role in picking_remaining_roles_for_approval and not view_only_roles and not requester_as_a_approver and not all_approvals_required and not status and  approvar_excits:
				...
				workflow_requests.reload()
				frappe.db.commit()
				sending_mail_api(request_id=request_id, doctype_name=document_type,property= property,cluster= cluster_name,reason= "Auto Approved by the system",timestamp= my_time,skip_user_role= current_user_role,user = frappe.session.user,field_changes=None,current_level=None )
