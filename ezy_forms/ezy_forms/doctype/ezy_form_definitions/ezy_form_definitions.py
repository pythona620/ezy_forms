...
def delete_roles_for_approver_roles(role:list|None,level:int|None,short_name:str|None, document_type:str|None, property:str|None):
	...
	level = int(level)
	count = 1
	form_definitions = frappe.get_all(
		"Ezy Form Definitions",
		filters={'form_name': document_type, "business_unit": property},
		pluck='name'
	)
 
	doc_name = form_definitions[0]
	form_def_json_str = frappe.get_value("Ezy Form Definitions", doc_name, "form_json")
	...
	roadmaps = frappe.get_all(
		"WF Roadmap",
		fields=["name"],
		filters={"document_type": short_name, "property": property}
	)

	updated_roadmaps = []

	for roadmap in roadmaps:
		doc = frappe.get_doc("WF Roadmap", roadmap.name)
