...
def add_roles_to_wf_requestors(business_unit:str,doctype:str,workflow_setup:list[dict]):
	try:
		...
		if approvers_section.shape[0]>0:
			approvers_section = approvers_section.select("roles","fields","idx","cancel_request","mandatory","action",'view_only_reportee','all_approvals_required','on_rejection').rename({"roles":"role","fields":"columns_allowed","idx":"level"}).to_dicts()
